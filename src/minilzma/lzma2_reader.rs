...
use super::{
    Read, error_invalid_input,
    lz_decoder::LzDecoder,
    lzma_decoder::LzmaDecoder,
    range_decoder::{RangeDecoder, RangeDecoderBuffer},
};
...
impl<R: Read> Lzma2Reader<R> {
    ...
    fn decode_chunk_header(&mut self) -> crate::Result<()> {
        ...
        if control >= 0xE0 || control == 0x01 {
            ...
            return Err(error_invalid_input("corrupted input data (LZMA2:0)").into());
        }
        if control >= 0x80 {
            ...
            if control >= 0xC0 {
                ...
                return Err(error_invalid_input("corrupted input data (LZMA2:1)").into());
            ...
            return Err(error_invalid_input("corrupted input data (LZMA2:2)").into());
        } else {
    ...
    fn decode_props(&mut self) -> crate::Result<()> {
        ...
        if props > (4 * 5 + 4) * 9 + 8 {
            return Err(error_invalid_input("corrupted input data (LZMA2:3)").into());
        }
        let pb = props / (9 * 5);
        ...
        let lc = props - lp * 9;
        if lc + lp > 4 {
            return Err(error_invalid_input("corrupted input data (LZMA2:4)").into());
        }
